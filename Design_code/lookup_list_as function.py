...
class Wave:
    ...
    def __init__(self, amplitude: float, period: float, direction: int):
...
class WaveSimulation:
    ...
    def combined_wave(self, x=None, t=None):
    ...
    def update(self, val):
        ...
        for i, line in enumerate(self.lines):
            line.set_ydata(self.waves[i].calculate_wave(self.x, self.t))
    ...
    def tick(self, delta=1, update=True):
        ...
        # Update Lines
        self.t += self.time_step * delta * 50  # Runs at 50fps in original code
        for i, line in enumerate(self.lines):
            line.set_ydata(self.waves[i].calculate_wave(self.x, self.t))
        self.line_sum.set_ydata(self.combined_wave())
        self.update_dots()
    ...
    def calculate_discrete_positions(self, count):
        ...
        servo_y_positions = self.combined_wave(servo_x_positions)
    ...
    async def setup_plot(self):
        ...
        for i, wave in enumerate(self.waves):
            ...
            amp_slider = Slider(
                ax_amp, f"Amplitude {i+1}", 0.1, 50.0, valinit=wave.amplitude
            )
            T_slider = Slider(ax_T, f"Period {i+1}", 4.0, 22.0, valinit=wave.period)
